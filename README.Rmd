...
knitr::opts_chunk$set(echo = TRUE, eval=FALSE, tidy = TRUE, tidy.opts = list(width.cutoff=60))
```

R package for graphs for Nephele 16S pipelines.

### Usage
- **Function reference:** [datavis16s R package manual](doc/Reference_Manual_datavis16s.rst)
...
-   There are 4 main functions for making graphs: `adivboxplot`,
    ...
    `allgraphs` which makes all 4. See the [manual](doc/Reference_Manual_datavis16s.md) for
    the arguments for these functions.
-   **Python with rpy2**
    ...
    -   The generic wrapper function to be called from rpy2 is
        [trygraphwrapper](doc/Reference_Manual_datavis16s.rst#trygraphwrapper).
        See the [function help](doc/Reference_Manual_datavis16s.rst#trygraphwrapper) for
        examples.
...
-   **Sampling depth**
    ...
    - For pipelines, the alpha diversity boxplot and the PCoA would benefit from having this sampling depth set properly.
        ...
        - All other functions will not remove samples or rarefy
                unless `sampdepth` is specified.

1Can optionally pass tab-delimited text file to each of the functions
instead of the biom file. See [`readindata`][doc/Reference_Manual_datavis16s.md#readindata] for
more details.
...
functions. The individual graph functions just use the specified output
directory passed to them. - Creates html files in outdir as well as
subdirectory "lib" which contains the external js,css,etc files for the
graphs. See [user
...
-   To build the package along with the user and package documentation,
    ...
    manual](doc/Reference_Manual_datavis16s.md) and the user docs [sphinx](datavis16s.user_doc.html) and [html for Nephele2 website](https://github.niaid.nih.gov/bcbb/nephele2/blob/master/pipelines/datavis16s/doc/datavis16s_pipeline.html).
